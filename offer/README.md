...
|55 - I. 二叉树的深度|树，深度优先遍历(easy)|同104|
|55 - II. 平衡二叉树|树，深度优先遍历(easy)|同110|
|56 - I. 数组中数字出现的次数|位运算(medium)|出现次数的题大多数都是用位运算，然而我不会|
|56 - II. 数组中数字出现的次数 II|位运算(medium)||
|57. 和为s的两个数字|(easy)||
|57 - II. 和为s的连续正数序列|(easy)|滑动窗口|
|58 - I. 翻转单词顺序|字符串(easy)|注意s.split(' ')与s.split()的区别|
|58 - II. 左旋转字符串|字符串(easy)||
|59 - I. 滑动窗口的最大值|滑动窗口，队列(easy)|使用队列来做|
|59 - II. 队列的最大值|栈|使用辅助队列|
|60. n个骰子的点数|动态规划(easy)|是个easy的题，结果难得很|
|61. 扑克牌中的顺子|(easy)|有很多解法，挺有意思的|
|62. 圆圈中最后剩下的数字|(easy)|约瑟夫环问题，需要数学推理|
|63. 股票的最大利润|动态规划(medium)|股票问题里最简单的一道，这一个系列还是有点难的，动态规划经典问题|
|64. 求1+2+…+n|(medium)|利用位运算|
|65. 不用加减乘除做加法|(easy)|可怕的位运算|
|66. 构建乘积数组|(medium)||
|67. 把字符串转换成整数|数学，字符串(medium)|处理起来需要考虑边界情况，比较复杂|
|68 - I. 二叉搜索树的最近公共祖先|树(easy)||
|68 - II. 二叉树的最近公共祖先|树(easy)||